...
public class MG_PerlinEnemy : MG_PerlinNoise
{
    ...
    protected override void FillTiles()
    {
        ...
        if (MiniMapImageTest)
        {
            ...
            Texture2D newTexture = CreateTextures2();
            MiniMapImageTest.sprite = Sprite.Create(newTexture, new Rect(0, 0, newTexture.width, newTexture.height), Vector2.zero);
            tSep = Time.realtimeSinceStartup - tSep;
            print("小地圖創建時間: " + tSep);
        }
    }

    Texture2D CreateTextures2()
    {
